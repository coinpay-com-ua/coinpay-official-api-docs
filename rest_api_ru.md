...
## Обработка Запросов
* Базовый api url - https://api.conpay.ua/
* Content-Type - json
...
1. Получение сколько есть "EUR" в "ETH" - balance[‘EUR’][‘ETH’]
...
На данный момент оплата такого счёта возможна только в случае, когда у пользователя, есть аккаунт на coinpay.ua и там хватает средств.

#### Этапы работы со счётом:

1. создаеться счёт(генериться ссылка для оплаты)
1. пользователь переходит по ссылке(логиниться на сайте coinpay.ua) и может оплатить в той валюте, в который был создан счёт, или любой другой валютой


#### Возможные статусы :

1. "NEW" - счёт создан
1. "PAYMENT_IN_PROGRESS" - оплата в процессе. Ожидание финального статуса
1. "CLOSED" - счёт учспешно оплачен
1. "ERROR" - счёт неудалось оплатить
1. "EXPIRED" - счёт прекратил существование. По истичении времени в 900 секунд счёт перестает быть активным
...
```
Для того, чтобы оплатить пользователю нужно пройти по указанной ссылке, залогиниться в "coinpay.ua", выбрать какой валютой оплатить, и пройти процесс оплаты
...
1. Обмен при достижении указанной цены - (под-тип заявки - "LIMIT_EXCHANGE")

При рыночном обмене возможны след.статусы:
1. "NEW" - заявка создана
1. "CLOSED" - обмен прошел успешно
1. "ERROR" - возникла ошибка, при обмене

При лимитном обмене возможны след.статусы:
1. "NEW", "WAITING_FOR_PRICE" - ожидание цены на платформе
1. "CLOSED" - обмен успешно произведен
1. "ERROR" - возникла ошибка при обмене
1. "CANCELLING" - обмен был отправлен на отмену
1. "CANCELLED" - обмен отменен
...
1. Нужно указать одну из сумм, или сделать обмен потратитв "X" валюты, или купить, чтобы получить "Y" валюты
1. Если есть желание купить по заказанной цене нужно указать значение параметра - "exchange_price"
1. Можно указать callback_url, чтобы подписаться на нотификации по изменению статуса обмена
